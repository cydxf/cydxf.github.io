...
// 设置 multer 用于文件上传
const upload = multer({ dest: 'Blog/resource/AI_tools/ASR/wav/' }); // 上传文件存储路径
...
wss.on('connection', ws => {
    ws.on('message', message => {
        ...
        if (data.command === 'start' && data.file) {
            let filePath = path.join('Blog/resource/AI_tools/ASR/wav', data.file);
            let outputFolder = path.join('Blog/resource/AI_tools/ASR/chunks');
            let outputTxtFile = path.join('Blog/resource/AI_tools/ASR/txt', data.file.replace('.wav', '.txt'));
