...
def process_chunk(chunk_path, index, results, progress_callback):
...
if __name__ == "__main__":
    ...
    # 确保输出目录存在
    output_txt_folder = os.path.dirname(output_txt_file)
    ...
    split_audio(converted_audio_file, 'chunks')
    ...
    with tqdm(total=len(chunk_files), desc="Processing") as pbar:
        with ThreadPoolExecutor(max_workers=50) as executor:
            future_to_chunk = {executor.submit(process_chunk, chunk_file, index, results, update_progress): (chunk_file, index) for index, chunk_file in enumerate(chunk_files)}

            for future in as_completed(future_to_chunk):
                try:
                    future.result()
                except Exception as e:
                    print(f"Error processing chunk {future_to_chunk[future][1]}: {e}")

    # 将结果写入指定路径的txt文件
    with open(output_txt_file, "w", encoding="utf-8") as f:
        for chunk_result in results:
            f.write(chunk_result.strip() + '\n')
